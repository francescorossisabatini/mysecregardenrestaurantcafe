...
export const LanguageSwitcher = ({ variant = "navbar", tone = "default" }: LanguageSwitcherProps) => {
  ...
  if (variant === "mobile") {
    return (
      <div className={`inline-flex h-11 shrink-0 items-center rounded-full border p-0.5 shadow-sm backdrop-blur-md ${isOverlay ? "border-border/75 bg-card/90" : "border-border/75 bg-card/90"}`} role="group" aria-label="Language selection">
        <button
