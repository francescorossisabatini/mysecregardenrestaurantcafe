...
const DietaryBadges = ({ text, language }: { text: string; language: "de" | "en" }) => {
  ...
  return (
    <div className="flex items-center gap-2 mt-2 flex-wrap">
      {labels.isVegan && (
        <span className="text-xs font-work font-semibold text-[color:var(--color-vegan)]">
          vegan
        </span>
      )}
      {labels.isGlutenFree && (
        <span className="text-xs font-work font-semibold text-[color:var(--color-gluten-free)]">
          {language === "de" ? "glutenfrei" : "gluten-free"}
        </span>
      )}
      {labels.isBio && (
        <span className="text-xs font-work font-semibold text-[color:var(--color-bio)]">
          bio
        </span>
      )}
    </div>
